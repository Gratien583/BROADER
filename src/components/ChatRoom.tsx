...
const ChatRoom: React.FC<ChatRoomProps> = ({ route }) => {
  ...
  const userCache = useRef(
    new Map<string, { username: string; icon: string | null }>()
  );
  ...
  const updateChatRoomStatus = async (isOpen: boolean) => {
    ...
    if (!chatRoomId || !userId) {
      console.log(
        "チャットルームIDまたはユーザーIDが無効です。chatRoomId:",
        chatRoomId,
        ", userId:",
        userId
      ); // 追加デバッグ: ID無効時の詳細表示
    ...
    try {
      ...
      if (error) {
        console.error(
          "チャットルームのステータス更新中にエラーが発生しました:",
          error
        );
      } else {
        console.log("チャットルームのステータスを更新しました:", {
          chatRoomId,
          userId,
          isOpen,
        }); // デバッグ: ステータス更新成功
      ...
      try {
        ...
        const updatedMessages = await Promise.all(
          messagesData.map(async (message: Message) => {
            ...
            if (userError) {
              console.error(
                `Error fetching user for message ${message.id}:`,
                userError
              );
            }
            return {
              ...message,
              username: userData?.username || "無名ユーザー",
        ...
        .on(
          ...
          async (payload) => {
            ...
            if (!userInfo) {
              ...
              if (userError) {
                console.error(
                  `Error fetching user for message ${newMessage.id}:`,
                  userError
                );
                return;
              }
            ...
            if (isChatRoomOpen && newMessage.user_id !== userId) {
              try {
                const openUsers = await fetchOpenChatRoomUsers(chatRoomId);
                const updatedReadByUserIds = openUsers.filter(
                  (id) => id !== userId
                );
                ...
                if (
                  !messageError &&
                  messageData &&
                  !messageData.read_by_user_ids.includes(userId)
                ) {
        ...
        .on(
          ...
          async (payload) => {
            ...
            if (!userInfo) {
              ...
              if (userError) {
                console.error(
                  `Error fetching user for message ${updatedMessage.id}:`,
                  userError
                );
                return;
              }
              ...
              userCache.current.set(updatedMessage.user_id, userInfo);
            }
  ...
  useEffect(() => {
    const markAllMessagesAsRead = async () => {
      ...
      try {
        ...
        if (error) {
          console.error(
            "未読メッセージの取得中にエラーが発生しました（Web）:",
            error
          );
          return;
        }

        // console.log('取得した未読メッセージ（Web）:', unreadMessages);
        ...
        if (unreadMessages && unreadMessages.length > 0) {
          for (const message of unreadMessages) {
            ...
            if (updateError) {
              console.error(
                "メッセージの既読処理中にエラーが発生しました（Web）:",
                updateError
              );
            } else {
              console.log(
                `メッセージID ${message.id} の既読処理が成功しました（Web）`
              );
            }
          }
        }
          ...
          if (!usersError && usersData) {
            ...
            usersData.forEach((user) => {
              if (user.pushNotificationToken) {
                console.log(
                  `通知送信先: ${user.id}, トークン: ${user.pushNotificationToken}`
                );
                sendPushNotification(user.pushNotificationToken, "", {
                  type: "message",
                });
              } else {
                console.warn(
                  `プッシュ通知トークンが設定されていないユーザー: ${user.id}`
                );
              }
            });
          } else {
  ...
  useEffect(() => {
    ...
    if (userId) {
      const tempOtherUserId =
        userId === route.params.hostUserId
          ? route.params.receiverUserId
          : route.params.hostUserId;
      setOtherUserId(tempOtherUserId || null);
      //console.log('otherUserIdが設定されました:', tempOtherUserId);
    }
  ...
  const renderMessageItem = ({
    ...
    const isJoinMessage =
      item.content && item.content.includes("が参加しました");
    const displayTime = isJoinMessage
    ...
    const handleLongPress = () => {
      ...
      Alert.alert("メッセージアクション", undefined, options, {
        cancelable: true,
      });
    };
        ...
        {item.user_id !== userId && (
          <View style={styles.messageHeader}>
            <Image
              source={
                item.icon
                  ? { uri: `${item.icon}?timestamp=${new Date().getTime()}` }
                  : require("../../assets/user_default_icon.png")
              }
              // @ts-ignore
                  ...
                  >
                    {item.read_by_user_ids.length > 0 && (
                      <Text style={styles.readReceipt}>
                        {activeTab === "personal" &&
                        item.read_by_user_ids.length === 1
                          ? "既読"
  ...
  return (
    <SafeAreaView style={styles.outerContainer}>
      ...
      <View
        style={[
          styles.header,
          Platform.OS === "web" ? styles.fixedHeader : null,
        ]}
      >
      ...
      {showFriendRequestModal && !isRequestSender && (
        <View style={styles.friendRequestContainer}>
          ...
          <View style={styles.friendRequestButtons}>
            <TouchableOpacity
              onPress={handleAllowFriendRequest}
      ...
      >
        ...
        <View
          style={[
            ...
            Platform.OS === "web" ? styles.fixedInputContainer : null,
          ]}
        >
          <TextInput
