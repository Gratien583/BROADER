...
import { WebMap } from '../components/Map.web';
import { LeafletMapScreen} from '../components/Map.ios';
...
const ArticleDetail: React.FC = () => {
  ...
  const MyComponent = () => {
    if (Platform.OS === 'web') {
      return <WebMap location={location} meetingplace={article.meeting_place} />;
    } else if(Platform.OS === 'ios'){
      return <LeafletMapScreen location={location} meetingplace={article.meeting_place}
       />;
    }else{
      return <Text>現在マップを使用できません</Text>
    }
  };
  ...
  sendPushNotification(pushNotificationToken, body,{
    type:"join_event",
  ...
  interface MapProps {
    meetingPlace: string; // {article.meeting_place} に対応
  }
