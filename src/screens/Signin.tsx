...
if (userData?.banned_until) {
  ...
  if (banDate > currentDate) {
    setSubmitError(`アカウントは一時的に利用できません。解除日は ${banDate.toLocaleDateString()} です。`);
    return;
  }
}
  ...
  } else if (roleData?.is_admin && Platform.OS !== 'ios' && Platform.OS !== 'android') {
    // 管理者かつモバイルでない場合AdminPageに遷移
    navigation.reset({
      index: 0,
      routes: [{ name: 'AdminPage' }],
    });
  } else {
    // 通常ユーザーまたはモバイルの場合の遷移先を切り替え
    const targetRoute = Platform.OS === 'web' ? 'Top' : 'EventlistTab';
