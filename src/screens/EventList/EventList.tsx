...
const fetchArticles = async () => {
  try {
    ...
    // フレンドIDとその属性を取得
    const friendIdsWithAttributes = friendData.map((friend) => ({
      friendId: friend.user_id === userId ? friend.friend_id : friend.user_id,
      attributes:
        friend.user_id === userId ? friend.attribute2 : friend.attribute1, // 自分から見た属性を選択
    }));

    // 現在の日時を取得
    const now = new Date().toISOString();

    if (friendIdsWithAttributes.length === 0) {
      setArticles([]); // フレンドがいない場合は空表示
      return;
    }
    ...
    const { data: articlesData, error: articlesError } = await supabase
      ...
      .or(
        friendIdsWithAttributes
          .map(({ friendId }) => `host_user_id.eq.${friendId}`)
          .join(',')
      )
    ...
    const filteredArticles = articlesData.filter((article) => {
      const hostId = article.host_user_id;
      const friend = friendIdsWithAttributes.find(
        (f) => f.friendId === hostId
      );
      ...
      // フレンドの属性と記事の属性を比較
      return friend.attributes?.some((attribute: string) =>
        article.attributes?.includes(attribute)
      );      
    });
